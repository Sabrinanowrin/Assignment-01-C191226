...
function calculateBudget() {
    function calculateBudget() {
        let sum = 0;
        for (let item of budgetList.children) {
            const valueString =
                item.children[0].children[1].children[0].innerHTML.replace(/,/g, "");
        ...
        totalBudget.innerHTML = formatMoney(sum);
    }
    calculateBudget();
    ...
    function deleteEntry() {
        ...
        // Approach 1:
        list.innerHTML -= deleteEntryHtml;

        // update total income value
        calculateIncome();
    }

    deleteExpenseButton.deleteEventListener("click", deleteEntry);
